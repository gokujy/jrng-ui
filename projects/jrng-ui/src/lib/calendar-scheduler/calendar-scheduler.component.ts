...
@Component({
  ...
  template: `
    ...
    >
      ...
      <div class="j-calendar-scheduler__grid" [class.j-calendar-scheduler__grid--day]="view() === 'day'" [class.j-calendar-scheduler__grid--week]="view() === 'week'">
          ...
          >
            ...
            <span class="j-calendar-scheduler__events">
              @for (event of day.events; track event.id) {
                <span class="j-calendar-scheduler__event" [style.--j-event-color]="event.color || null" (click)="handleEventClick($event, event, day.date)">
                  {{ event.title }}
                </span>
              }
            </span>
...
export class JCalendarSchedulerComponent {
  ...
  setView(event: Event): void {
    const value = (event.target as HTMLSelectElement | null)?.value as JCalendarSchedulerView | undefined;
    if (!value) {
      return;
    }
    this.view.set(value);
