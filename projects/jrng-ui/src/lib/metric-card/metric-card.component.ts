...
import { JStatTrend } from '../stat-card/stat-card.component';
...
@Component({
  ...
  template: `
    ...
    >
      @if (loading()) {
        ...
        @if (trendLabel()) {
          <span class="j-metric-card__trend" [class]="'j-metric-card__trend j-metric-card__trend--' + trend()">
            {{ trendLabel() }}
          </span>
        }
        @if (footer()) {
  ...
  styles: [
    `
      .j-metric-card {
        background:
          linear-gradient(180deg, color-mix(in srgb, var(--j-color-card) 92%, var(--j-color-primary) 8%), var(--j-color-card));
...
export class JMetricCardComponent {
  ...
  readonly cardClasses = computed(() => ['j-metric-card', this.styleClass()].filter(Boolean).join(' '));
}
