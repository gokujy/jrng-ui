...
function jResolveSelectionLabel(
  ...
  if (jIsSelectionRecord(option)) {
    return String(option[optionLabel] ?? option['label'] ?? option[optionValue] ?? option['value'] ?? '');
  }
...
function jResolveSelectionValue(
  ...
  if (jIsSelectionRecord(option)) {
    return option[optionValue] ?? option['value'] ?? option[optionLabel] ?? option['label'] ?? option;
  }
