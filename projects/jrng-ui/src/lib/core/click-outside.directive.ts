...
import { DestroyRef, Directive, ElementRef, PLATFORM_ID, Renderer2, inject, output } from '@angular/core';
...
export class JClickOutsideDirective {
  ...
  constructor() {
    ...
    const removeMouseListener = this.renderer.listen(this.documentRef, 'mousedown', (event: MouseEvent) => {
      this.handleDocumentPointer(event);
    });
    const removeTouchListener = this.renderer.listen(this.documentRef, 'touchstart', (event: TouchEvent) => {
      this.handleDocumentPointer(event);
    });
