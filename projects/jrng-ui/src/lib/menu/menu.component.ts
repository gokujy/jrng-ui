            ...
            >
              ...
              >
                @if (itemTemplate) {
                  ...
                  @if (item.icon) {
                    <span class="j-menu__icon" data-jc-section="icon" aria-hidden="true">{{ item.icon }}</span>
                  }
              ...
              @if (item.items?.length && submenuOpen(path)) {
                <ng-container
                  [ngTemplateOutlet]="menuList"
                  [ngTemplateOutletContext]="{ items: item.items, level: level + 1, parentPath: path }"
                />
              }
            </li>
          }
...
export class JMenuComponent {
  ...
  @ContentChild('jMenuItem', { read: TemplateRef }) itemTemplate?: TemplateRef<JMenuItemTemplateContext>;
  ...
  private move(direction: 1 | -1): void {
    ...
    const current = Math.max(0, enabled.findIndex((entry) => entry.path === this.activePath));
  ...
  private handleTypeahead(event: KeyboardEvent): void {
    ...
    const next = this.typeahead.search(
      event.key,
      enabled.map((entry) => ({ item: entry.item, text: entry.item.label ?? '', disabled: entry.item.disabled })),
      current,
    );
    if (next >= 0) {
      event.preventDefault();
  ...
  private activeButtonIndex(): number {
    return Math.max(0, this.flatEnabledItems().findIndex((entry) => entry.path === this.activePath));
  }
  ...
  private flatEnabledItems(items = this.model, parentPath = 'root'): { readonly item: JMenuItem; readonly path: string }[] {
    return items.flatMap((item, index) => {
      const path = this.itemKey(item, parentPath, index);
      if (item.separator) {
        return [];
      }
      const children = item.items?.length && this.openPaths.has(path) ? this.flatEnabledItems(item.items, path) : [];
