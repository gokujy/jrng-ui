  ...
  template: `
    ...
    >
      <div class="j-virtual-scroller__spacer" [style.height.px]="totalHeight">
        <div class="j-virtual-scroller__content" [style.transform]="'translateY(' + offsetY + 'px)'">
          @if (loading) {
...
export class JVirtualScrollerComponent<T = unknown> {
  ...
  @ContentChild('jVirtualScrollerItem', { read: TemplateRef }) itemTemplate?: TemplateRef<JVirtualScrollerItemContext<T>>;
  ...
  trackItem(item: T, index: number): unknown {
    return typeof item === 'object' && item != null ? (item as Record<string, unknown>)['id'] ?? index : index;
  }
}
