...
export type JDialogPosition = 'center' | 'top' | 'bottom' | 'left' | 'right' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right';
...
export class JrDialogComponent implements OnDestroy {
  ...
  private dragStart: { readonly x: number; readonly y: number; readonly left: number; readonly top: number } | null = null;
  private resizeStart: { readonly x: number; readonly y: number; readonly width: number; readonly height: number } | null = null;
