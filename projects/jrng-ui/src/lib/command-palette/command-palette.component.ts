...
import { JDialogCloseReason, JrDialogComponent } from '../dialog/dialog.component';
...
export class JCommandPaletteComponent {
  ...
  itemClasses(item: JCommandPaletteItem): string {
    return ['j-command-palette__item', this.activeItem() === item ? 'is-active' : ''].filter(Boolean).join(' ');
  }
}
