import { ChangeDetectionStrategy, Component, DestroyRef, TemplateRef, booleanAttribute, contentChild, effect, inject, input, model, numberAttribute } from '@angular/core';
...
@Component({
  ...
  template: `
    <section class="j-carousel" [class]="styleClass()" data-jc-name="carousel" data-jc-section="root">
      <div class="j-carousel__viewport" [style.--j-carousel-items]="visibleItems()">
        @for (item of value(); track item.image || item.title || $index; let index = $index) {
          <article class="j-carousel__item" [class.is-active]="index === activeIndex()" [style.transform]="transform()">
      ...
      @if (controls()) {
        <button class="j-carousel__control j-carousel__control--previous" type="button" (click)="previous()" aria-label="Previous">&lt;</button>
        <button class="j-carousel__control j-carousel__control--next" type="button" (click)="next()" aria-label="Next">&gt;</button>
      }

      @if (indicators()) {
        <div class="j-carousel__indicators" data-jc-section="indicators">
          @for (item of value(); track item.image || item.title || $index; let index = $index) {
            <button type="button" [class.is-active]="index === activeIndex()" [attr.aria-label]="'Go to item ' + (index + 1)" (click)="activeIndex.set(index)"></button>
          }
        </div>
      }
    </section>
  ...
  styles: [
    `
      ...
      .j-carousel__viewport {
        display: grid;
        gap: var(--j-spacing-3);
        grid-auto-columns: calc((100% - (var(--j-carousel-items, 1) - 1) * var(--j-spacing-3)) / var(--j-carousel-items, 1));
...
export class JCarouselComponent {
  ...
  readonly itemTemplate = contentChild<unknown, TemplateRef<JCarouselItemContext>>('jCarouselItem', { read: TemplateRef });
