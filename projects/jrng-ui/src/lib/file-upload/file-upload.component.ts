    ...
    >
      ...
      <div class="j-file-upload__header" data-jc-section="header">
        ...
        <div class="j-file-upload__actions">
          <button type="button" class="j-file-upload__button" [disabled]="disabled()" (click)="openFilePicker()">
            {{ chooseLabel() }}
          </button>
          @if (!auto()) {
            <button
          ...
          @if (queue().length) {
            <button type="button" class="j-file-upload__button" [disabled]="disabled()" (click)="clear()">
              {{ cancelLabel() }}
            </button>
          }
        </div>
          ...
          @for (item of queue(); track item.id) {
            <li [attr.data-j-active]="item.status === 'uploading' ? 'true' : null">
              ...
              <div class="j-file-upload__meta">
                ...
                <span class="j-file-upload__progress"><span [style.width.%]="item.progress"></span></span>
                @if (item.error) {
...
export class JFileUploadComponent {
  ...
  readonly uploaderClasses = computed(() =>
    ['j-file-upload', this.mode() === 'basic' ? 'j-file-upload--basic' : '', this.styleClass()].filter(Boolean).join(' '),
  );
  ...
  setProgress(itemId: string, progress: number): void {
    this.updateItem(itemId, { progress: Math.min(100, Math.max(0, progress)), status: progress >= 100 ? 'complete' : 'uploading' });
  }
