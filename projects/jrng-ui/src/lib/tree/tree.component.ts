        ...
        @for (node of nodes; track node.key || node.label || $index; let index = $index) {
          ...
          >
            ...
            >
              ...
              >
                @if (nodeTemplate(); as template) {
                  <ng-container [ngTemplateOutlet]="template" [ngTemplateOutletContext]="nodeContext(node, key, level)" />
                } @else {
            ...
            @if (hasChildren(node) && isExpandedKey(key)) {
              <ng-container
                [ngTemplateOutlet]="treeList"
                [ngTemplateOutletContext]="{ nodes: filteredChildren(node), level: level + 1, parentKey: key }"
              />
            }
          </li>
        } @empty {
...
export class JTreeComponent {
  ...
  readonly nodeTemplate = contentChild<unknown, TemplateRef<JTreeNodeContext>>('jTreeNode', { read: TemplateRef });

  readonly filterValue = signal('');
  readonly activeKey = signal('');
  private readonly expandedKeys = signal<ReadonlySet<string>>(new Set());

  readonly visibleNodes = computed(() => (this.filterValue() ? this.filterNodes(this.value()) : this.value()));
  ...
  handleKeydown(event: KeyboardEvent): void {
    ...
    const current = Math.max(0, flat.findIndex((entry) => entry.key === this.activeKey()));

    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      const next = event.key === 'ArrowDown' ? Math.min(flat.length - 1, current + 1) : Math.max(0, current - 1);
  ...
  iconFor(node: JTreeNode, key: string): string {
    return this.isExpandedKey(key) ? node.expandedIcon || node.icon || '' : node.collapsedIcon || node.icon || '';
  }

  filteredChildren(node: JTreeNode): readonly JTreeNode[] {
    return this.filterValue() && node.children ? this.filterNodes(node.children) : node.children ?? [];
  }
  ...
  private flatten(nodes: readonly JTreeNode[], level = 1, parentKey = 'root'): JTreeFlatEntry[] {
    return nodes.flatMap((node, index) => {
      ...
      const children = this.isExpandedKey(key) ? this.flatten(node.children ?? [], level + 1, key) : [];
