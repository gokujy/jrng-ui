...
import { JTableColumn } from '../table/table.types';
import { JTreeNode, JTreeSelectionMode } from '../tree/tree.component';
...
@Component({
  ...
  template: `
    <section class="j-tree-table" [class]="styleClass()" data-jc-name="tree-table" data-jc-section="root">
      @if (filter()) {
        <label class="j-tree-table__filter" data-jc-section="filter">
          <span class="j-hidden-accessible">Filter tree table</span>
          <input type="search" [value]="filterValue()" [placeholder]="filterPlaceholder()" (input)="setFilter($event)" />
        </label>
      }
            ...
            <tr>
              ...
              @for (column of columns(); track column.field) {
                <th scope="col" [attr.aria-sort]="ariaSort(column)">
                  @if (column.sortable) {
                    <button class="j-tree-table__sort" type="button" (click)="toggleSort(column)">{{ column.header }}</button>
                  } @else {
                    {{ column.header }}
                  }
                </th>
          ...
          <tbody>
            @for (entry of visibleEntries(); track entry.key) {
              <tr
                ...
                [attr.aria-expanded]="hasChildren(entry.node) ? expandedKeys().has(entry.key) : null"
              ...
              >
                @if (selectionMode() === 'checkbox') {
                  <td class="j-tree-table__select-cell">
                    <input type="checkbox" [checked]="isSelected(entry.node)" [disabled]="entry.node.disabled" (change)="toggleSelection(entry.node, $event)" />
                  </td>
                }
                @for (column of columns(); track column.field; let columnIndex = $index) {
                  <td [attr.data-label]="column.header">
                    <div class="j-tree-table__cell" [style.padding-left.rem]="columnIndex === 0 ? (entry.level - 1) * 1.25 : 0">
                      @if (columnIndex === 0) {
                        <button
                          class="j-tree-table__toggle"
                          type="button"
                          [disabled]="!hasChildren(entry.node) || entry.node.disabled"
                          [attr.aria-label]="expandedKeys().has(entry.key) ? 'Collapse row' : 'Expand row'"
                      ...
                      @if (cellTemplate(); as template) {
                        <ng-container [ngTemplateOutlet]="template" [ngTemplateOutletContext]="cellContext(entry.node, column, entry.level)" />
                      } @else {
              ...
              <tr>
                <td class="j-tree-table__empty" [attr.colspan]="columns().length + (selectionMode() === 'checkbox' ? 1 : 0)">
                  {{ emptyMessage() }}
                </td>
              </tr>
            }
...
export class JTreeTableComponent {
  ...
  readonly cellTemplate = contentChild<unknown, TemplateRef<JTreeTableCellContext>>('jTreeTableCell', { read: TemplateRef });
  ...
  readonly visibleEntries = computed(() => {
    ...
    return [...entries].sort(
      (a, b) =>
        String(this.cellValue(a.node, { field: this.sortField(), header: this.sortField() })).localeCompare(
  ...
  toggleSort(column: JTableColumn): void {
    const nextOrder = this.sortField() !== column.field ? 1 : this.sortOrder() === 1 ? -1 : this.sortOrder() === -1 ? 0 : 1;
  ...
  cellValue(node: JTreeNode, column: JTableColumn): unknown {
    ...
    return typeof node.data === 'object' && node.data != null ? (node.data as Record<string, unknown>)[column.field] : '';
  }
  ...
  private flatten(nodes: readonly JTreeNode[], level = 1, parent = 'root'): JTreeTableEntry[] {
    return nodes.flatMap((node, index) => {
      ...
      const children = this.expandedKeys().has(key) ? this.flatten(node.children ?? [], level + 1, key) : [];
  ...
  private isNodeArray(value: JTreeNode | readonly JTreeNode[] | null): value is readonly JTreeNode[] {
