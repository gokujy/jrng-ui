...
export type JPassThroughAttributes = Readonly<
  Record<
    string,
    JPassThroughValue | readonly JPassThroughValue[] | Record<string, boolean | null | undefined>
  >
>;
...
export function jPassThroughFor(
  pt: JPassThrough | null | undefined,
  part: string,
): JPassThroughAttributes {
  return pt?.[part] ?? {};
}
...
export function jMergePartClasses(
  ...
  return jMergeClasses(
    componentClasses,
    styleClass,
    jPassThroughFor(pt, part)['class'] as JClassValue,
  );
}
