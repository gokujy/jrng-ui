...
const registry = await readFile(resolve(workspace, 'projects/docs/src/app/docs/generated-component-registry.ts'), 'utf8');
...
if (selectors.length !== inventory.summary.publicComponents || missing.length) {
  throw new Error(`Documentation registry mismatch: ${selectors.length} records; missing ${missing.join(', ') || 'none'}.`);
}
