...
| Component           | Selector                | Category  | Existing examples | Confusion found              | Missing API coverage     | Route                  | Navigation | Required action             | Baseline status |
| ------------------- | ----------------------- | --------- | ----------------- | ---------------------------- | ------------------------ | ---------------------- | ---------- | --------------------------- | --------------- |
...
- Forbidden reference labels and `j-data-grid` references are absent from runtime documentation source.
- API coverage remains complete for 122 components, 1,446 inputs, 241 outputs, and 713 registry methods.
...
- Added `docs:validate-content` for forbidden reference labels, removed selectors, third-party identity leakage, duplicate feature/API example keys, preview/source-tab wiring, copy-code metadata, modular imports, and the required independent Table filtering contracts.
...
- All 122 public components and documentation records are complete under the focused preview and API-example standard.
- Final coverage is 1,091 runnable examples with zero remaining API-example gaps, broken routes, duplicate routes, hidden implemented advanced components, or forbidden business labels.
- Library, documentation, unit, lint, route, content, public API, package, consumer, SSR, theme, enterprise-audit, and browser E2E validation pass.
- The visual regression manifest now covers Timeline, Stepper, and Empty, bringing the required fixture set to 34 components.
- Table browser validation passes at 320, 375, 768, and 1024 pixels and for frozen columns, RTL, column resize, and column reorder.
- The final results, commit ledger, package impact, warnings, and integrity confirmations are recorded in `docs/planning/docs-rebuild-completion.md`.
